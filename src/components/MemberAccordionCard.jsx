...
export default function MemberAccordionCard({ member, onApproveProject, onRequestFeedback }) {
  ...
  if (!member) return null; // defensive: never render on a missing member object

  const name = member.name ?? '이름 없음';
  const role = member.role ?? '';
  const status = member.status ?? '미작성';
  const projects = Array.isArray(member.projects) ? member.projects : [];
  const statusClass = status === '제출완료' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700';
    ...
    <div className="rounded-xl border border-surface-200 bg-white overflow-hidden">
      ...
      >
        ...
        <div className="min-w-[120px]">
          <p className="text-sm font-semibold">
            {name} <span className="text-navy-400 font-normal">· {role}</span>
          </p>
          <p className="text-xs text-navy-400">{projects.length}개 프로젝트 진행 중</p>
        </div>
      ...
      {isExpanded && (
        <div className="border-t border-surface-200 p-4 sm:p-5 space-y-3">
          {projects.length === 0 ? (
            <div className="rounded-lg border border-dashed border-surface-200 py-8 text-center">
              <p className="text-sm text-navy-400">작성된 일일보고가 없습니다.</p>
            </div>
          ) : (
